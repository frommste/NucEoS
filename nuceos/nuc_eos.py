...
class BetaEquilibriumEOS(EOSBase):
    ...
    def __init__(self, eos_table_file, T0=0.0):
        ...
        for irho in range(self.logrho.size):
            press_func = interp1d(munu[:, iT0, irho], logpress[:, iT0, irho])
            energy_func = interp1d(munu[:, iT0, irho], logenergy[:, iT0, irho])
            ye_func = interp1d(munu[:, iT0, irho], ye[:, iT0, irho])
        ...
        self.logpress = _np.exp(convert(10.0**self.logpress, CGS, REL))
        self.logenergy = _np.exp(convert(10.0**self.logenergy, CGS, REL))
        self.logrho = _np.exp(convert(10.0**self.logrho, CGS, REL))
    ...
    def from_density(self, rho):
        ...
        assert self.rho_min < rho < self.rho_max, \
            "Provided density is outside of EOS table range."
